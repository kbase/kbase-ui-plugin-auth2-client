    ...
    class UseAgreements extends Component {
        ...
        togglePolicyAgreement(policy) {
            const agreedTo = (() => {
                ...
                return this.state.agreedTo.concat([policy]);
        ...
        renderMissingAgreements() {
            if (this.props.policiesToResolve.missing.length === 0) {
                return;
            }
            const missingAgreements = this.props.policiesToResolve.missing.map(({id, version, policy}) => {
        ...
        timeStyle: 'short',
        hour12: true
    }).format(new Date(policy.date))}</div>
                                </div>
        ...
        render() {
            if (this.props.policiesToResolve.missing.length + this.props.policiesToResolve.outdated.length === 0) {
                return;
            }
            return html`
                <div className="UseAgreements">
                    ${this.renderMissingAgreements()}
                    ${this.renderOutdatedAgreements()}
                </div>
            `;
        }
    }
